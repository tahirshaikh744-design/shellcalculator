...
class VoltageCalculatorActivity : BaseCalculatorActivity() {
    ...
    private var unit1Factor = 1.0 // Volt default
    private var unit2Factor = 0.001 // Millivolt default

    private val units = arrayOf(
        VoltageUnit("Volt", "V", 1.0),
        VoltageUnit("Millivolt", "mV", 0.001),
        VoltageUnit("Microvolt", "µV", 1e-6),
        VoltageUnit("Kilovolt", "kV", 1000.0),
        VoltageUnit("Megavolt", "MV", 1e6),
        VoltageUnit("Abvolt", "abV", 1e-8),
        VoltageUnit("Statvolt", "statV", 299.7925)
    )
    ...
    private fun showUnitPicker(onSelected: (VoltageUnit) -> Unit) {
    ...
    data class VoltageUnit(val name: String, val symbol: String, val factor: Double)
}
