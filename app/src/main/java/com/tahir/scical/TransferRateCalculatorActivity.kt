...
class TransferRateCalculatorActivity : BaseCalculatorActivity() {
    ...
    private var unit1Factor = 1.0 // Mbps default
    private var unit2Factor = 0.125 // MB/s default

    private val units = arrayOf(
        TransferUnit("Bit per second", "bps", 1e-6),
        TransferUnit("Kilobit per second", "kbps", 0.001),
        TransferUnit("Megabit per second", "Mbps", 1.0),
        TransferUnit("Gigabit per second", "Gbps", 1000.0),
        TransferUnit("Terabit per second", "Tbps", 1e6),
        TransferUnit("Byte per second", "B/s", 8e-6),
        TransferUnit("Kilobyte per second", "KB/s", 0.008),
        TransferUnit("Megabyte per second", "MB/s", 8.0),
        TransferUnit("Gigabyte per second", "GB/s", 8000.0),
        TransferUnit("Terabyte per second", "TB/s", 8e6)
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvHeaderTitle.text = "Transfer Rate"
        ...
        tvUnit1Name.text = units[2].name
        tvUnit1Symbol.text = units[2].symbol
        unit1Factor = units[2].factor

        tvUnit2Name.text = units[7].name
        tvUnit2Symbol.text = units[7].symbol
        unit2Factor = units[7].factor
    ...
    private fun showUnitPicker(onSelected: (TransferUnit) -> Unit) {
    ...
    data class TransferUnit(val name: String, val symbol: String, val factor: Double)
}
