...
class RadiationCalculatorActivity : BaseCalculatorActivity() {
    ...
    private var unit1Factor = 1.0 // Gray
    private var unit2Factor = 100.0 // Rad

    private val units = arrayOf(
        RadUnit("Gray", "Gy", 1.0),
        RadUnit("Rad", "rad", 0.01),
        RadUnit("Milligray", "mGy", 0.001),
        RadUnit("Microgray", "µGy", 1e-6),
        RadUnit("Sievert", "Sv", 1.0),
        RadUnit("Rem", "rem", 0.01)
    )
    ...
    private fun showUnitPicker(onSelected: (RadUnit) -> Unit) {
    ...
    data class RadUnit(val name: String, val symbol: String, val factor: Double)
}
